...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            ...
            $modx->addPackage('PhpConsole\Model', MODX_CORE_PATH . 'components/phpconsole/src/', null, 'PhpConsole\\');
            $manager = $modx->getManager();
            $objects = [];
            $schemaFields = [];
            $schemaFile = MODX_CORE_PATH . 'components/phpconsole/schema/phpconsole.mysql.schema.xml';
            if (is_file($schemaFile)) {
                $schema = new SimpleXMLElement($schemaFile, 0, true);
                if (isset($schema->object)) {
                    foreach ($schema->object as $obj) {
                        $class = 'PhpConsole\\Model\\' . (string)$obj['class'];
                        $objects[] = $class;
                        $schemaFields[$class] = [];
